...
export class DFU {
  ...
  private progress(task: string, current: number, total: number) {
    if (this.progressCallback) this.progressCallback({ task, current, total });
  }
  ...
  private async getChipInfo() {
    ...
    return descriptors.map(parseDescriptor).reduce(function (o: any, v, i) {
  ...
  private async loadAddress(address: number, abort = true) {
    ...
    const busyCheck = await this.controlTransferIn(
  ...
  private async unlockOptionBytes(start_address: number, total_size: number) {
    const unprotect = async () => {
      ...
      const status = await this.controlTransferIn(
        DFURequest.GETSTATUS,
        0,
        0,
        6
    ...
    const tryReadOB = async () => {
      ...
      const ob_data = await this.controlTransferIn(
        DFURequest.UPLOAD,
        2,
        0,
        total_size
    ...
    if (
      loadAddressResponse[4] == DFUState.dfuERROR &&
      loadAddressResponse[0] == DFUStatus.errVENDOR
    ) {
      // read protected
      await this.clearStatus();
      return unprotect();
  ...
  private async erasePages(flash_layout: any) {
    ...
    for (const erase_page of erase_pages) {
      ...
      const page_addr =
        erase_page.page * flash_layout.sectors[erase_page.sector].page_size +
        flash_layout.sectors[erase_page.sector].start_address;
      const cmd = [
        0x41,
        page_addr & 0xff,
        (page_addr >> 8) & 0xff,
      ...
      if (data[4] == DFUState.dfuDNLOAD_IDLE) {
        ...
        const clearStatus = await this.controlTransferIn(
          DFURequest.GETSTATUS,
          0,
          0,
          6
  ...
  private async upload(hex: IntelHEX, transferSize: number) {
    ...
    for (const segment of hex.segments) {
      ...
      while (bytes_flashed < segment.data.length) {
        ...
        this.controlTransferOut(DFURequest.DNLOAD, wBlockNum++, 0, [
          ...data_to_flash,
        ]);
